''' This script in replacement of NIS server usercreation to generate unix uid for new user creation in
inventory server. The usage has been explained in the parser '''
import argparse
import datetime
import csv
import sys
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='NIS usercreation substitute for generating unix UID')
    sub_parser = parser.add_subparsers(help='NIS UserCreation substitute to generate Unix UID')

    create_subparser = sub_parser.add_parser('create', help='Usage Eg: test5.py create -u mapi2506 -g xx@unilever.com -r hclunix, -t REQ84848')
    create_subparser.add_argument('-u', '--user', type=str,  help='Provide unix user name as per UL standard', required=True)
    create_subparser.add_argument('-g', '--gecos', type=str, help='Provide email address', required=True)
    create_subparser.add_argument('-r', '--rpm', type=str, help="Provide the group Eg: hclunix", required=True)
    create_subparser.add_argument('-t', '--ticket', type=str, help="Provide the Service Request Eg: REQ773737", required=True)
